...
export const useReports = () => {
  ...
  const getSalesStats = useCallback(() => {
    ...
    const totalRevenue = salesData.reduce((sum, sale) => sum + (sale.total_amount || sale.total || 0), 0);  // ✅ total_amount
    ...
    const todayRevenue = todaySales.reduce((sum, sale) => sum + (sale.total_amount || sale.total || 0), 0);  // ✅ total_amount
